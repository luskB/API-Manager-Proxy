...
import {ArrowLeftRight, Copy, Globe, Key, LogIn, Pencil, Plus, RefreshCw, ToggleLeft, ToggleRight, Trash2, Upload, X,} from "lucide-react";
...
interface BrowserLoginInfo {
    site_url: string;
    debug_port: number;
    profile_mode: "main" | "isolated";
    profile_path: string;
    chrome_path: string;
}

interface BrowserLoginImportResult {
...
function hasUsableManualApiKey(value: string): boolean {
...
function toTokenText(value: unknown): string {
...
function tokenId(token: JsonRecord): string {
    return (
        toTokenText(token.id) ||
        toTokenText(token.token_id) ||
        toTokenText(token.key) ||
        toTokenText(token.name) ||
        crypto.randomUUID()
    );
}

function tokenKeyValue(token: JsonRecord): string {
    return toTokenText(token.key) || toTokenText(token.token);
}

function maskTokenKey(value: string): string {
    if (!value) return "-";
...
export default function AccountsPage() {
  ...
  const [browserSyncStatus, setBrowserSyncStatus] = useState<"" | "detecting" | "syncing" | "fetching_keys">("");
    ...
    const [launchingBrowserLogin, setLaunchingBrowserLogin] = useState(false);
    const [importingBrowserLogin, setImportingBrowserLogin] = useState(false);
    const [browserProfileMode, setBrowserProfileMode] = useState<"main" | "isolated">("main");
  const [validating, setValidating] = useState(false);
    ...
    const [apiKeySwitcherAccount, setApiKeySwitcherAccount] = useState<SiteAccount | null>(null);
    const [apiKeySwitcherLoading, setApiKeySwitcherLoading] = useState(false);
    const [apiKeySwitcherApplying, setApiKeySwitcherApplying] = useState("");
    const [apiKeySwitcherError, setApiKeySwitcherError] = useState("");
    const [apiKeySwitcherTokens, setApiKeySwitcherTokens] = useState<JsonRecord[]>([]);
  ...
  const { t, locale } = useLocale();
    ...
    const closeAccountModal = useCallback(() => {
        if (validating || launchingBrowserLogin || importingBrowserLogin) return;
        setAccountModalOpen(false);
        setAccountModalTargetId(null);
        setAccountModalError("");
        setBrowserLoginInfo(null);
        setBrowserProfileMode("main");
    }, [importingBrowserLogin, launchingBrowserLogin, validating]);
    ...
    }, [accountModalOpen, closeAccountModal]);
  ...
  function copyApiKey(account: SiteAccount) {
    ...
    async function saveEditedApiKey(accountId: string) {
        ...
        try {
            ...
            const autoEnable = Boolean(targetAccount?.disabled) && hasUsableManualApiKey(nextKey);
            const updatedAccounts = config.accounts.map((a) =>
                a.id === accountId
                    ? {
                        ...a,
                        disabled: autoEnable ? false : a.disabled,
                        account_info: {
                            ...a.account_info,
                            api_key: nextKey,
                        },
    ...
    async function openApiKeySwitcher(account: SiteAccount) {
        setApiKeySwitcherAccount(account);
        setApiKeySwitcherError("");
        setApiKeySwitcherTokens([]);
        setApiKeySwitcherApplying("");
        setApiKeySwitcherLoading(true);
        try {
            const tokens = await request<JsonRecord[]>("hub_fetch_api_tokens", {
                account_id: account.id,
            });
            setApiKeySwitcherTokens(tokens ?? []);
        } catch (e) {
            setApiKeySwitcherError(String(e));
        } finally {
            setApiKeySwitcherLoading(false);
        }
    }

    function closeApiKeySwitcher() {
        if (apiKeySwitcherApplying) return;
        setApiKeySwitcherAccount(null);
        setApiKeySwitcherError("");
        setApiKeySwitcherTokens([]);
        setApiKeySwitcherLoading(false);
        setApiKeySwitcherApplying("");
    ...
    async function applySwitchedApiKey(token: JsonRecord) {
        if (!config || !apiKeySwitcherAccount || apiKeySwitcherApplying) return;

        const nextKey = tokenKeyValue(token).trim();
        if (!nextKey || nextKey.includes("*")) {
            setApiKeySwitcherError(
                text(
                    "This token only exposes a masked key and cannot be used directly.",
                    "This token only exposes a masked key and cannot be used directly.",
                ),
            );
            return;
        ...
        setApiKeySwitcherApplying(tokenId(token));
        setApiKeySwitcherError("");
        setImportStatus("");

        try {
            const updatedAccounts = config.accounts.map((account) =>
                account.id === apiKeySwitcherAccount.id
                    ? {
                        ...account,
                        disabled: false,
                        account_info: {
            ...
            const updated: AppConfig = {
                ...config,
                accounts: updatedAccounts,
                proxy_accounts: updatedAccounts.filter((account) => !account.disabled),
            };
            await request("save_config", { config_data: updated });
            setConfig(updated);
            setImportStatus(
                text("Switched the site API key.", "Switched the site API key."),
            );
            closeApiKeySwitcher();
        } catch (e) {
            setApiKeySwitcherError(String(e));
  ...
  async function handleRefreshKeys() {
    ...
    try {
      ...
      setImportStatus(
        text(
            `Refreshed missing API keys: ${result.success} updated, ${result.preserved} preserved, ${result.failed} failed, ${result.skipped} skipped`,
            `Refreshed missing API keys: ${result.success} updated, ${result.preserved} preserved, ${result.failed} failed, ${result.skipped} skipped`,
        )
      );
      await reload();
    ...
    function openEditAccountModal(account: SiteAccount) {
        ...
        setAccountModalError("");
        setBrowserLoginInfo(null);
        setBrowserProfileMode((account.browser_profile_mode as "main" | "isolated") || "main");
        setAccountModalOpen(true);
  }
    ...
    function openDuplicateAccountModal(account: SiteAccount) {
        ...
        setAccountModalError("");
        setBrowserLoginInfo(null);
        setBrowserProfileMode((account.browser_profile_mode as "main" | "isolated") || "isolated");
        setAccountModalOpen(true);
    }
                ...
                return config.accounts.map((a) =>
                    a.id === accountModalTargetId
                        ? {
                            ...
                            disabled:
                                a.disabled && hasUsableManualApiKey(key)
                                    ? false
                                    : a.disabled,
    ...
    async function handleOpenBrowserLogin() {
        ...
        setLaunchingBrowserLogin(true);
        setAccountModalError("");
        try {
            const info = await request<BrowserLoginInfo>("open_browser_login", {
                site_url: url,
                profile_mode: browserProfileMode,
            });
            setBrowserLoginInfo(info);
            setImportStatus(t("accounts.browserLoginOpened"));
        } catch (e) {
            setAccountModalError(String(e));
        } finally {
            setLaunchingBrowserLogin(false);
        }
    }

    async function handleImportFromBrowserLogin() {
        if (!config) return;
        const url = accountForm.siteUrl.trim();
        if (!url) {
            setAccountModalError(t("accounts.siteUrlRequired"));
        ...
        if (!browserLoginInfo?.debug_port) {
            setAccountModalError(t("accounts.browserLoginNeedOpen"));
            return;
        }

        setImportingBrowserLogin(true);
        setAccountModalError("");
        try {
            const result = await request<BrowserLoginImportResult>("import_account_from_browser_login", {
                site_url: url,
                site_type: accountForm.siteType,
                debug_port: browserLoginInfo.debug_port,
                profile_mode: browserProfileMode,
                site_name: accountForm.siteName.trim() || undefined,
                disabled: accountModalMode === "duplicate" ? true : false,
            });
            ...
            setImportStatus(
                result.action === "updated"
                    ? t("accounts.browserImportUpdated")
                    : t("accounts.browserImportSuccess"),
            );
        } catch (e) {
            setAccountModalError(String(e));
        } finally {
            setImportingBrowserLogin(false);
        }
    }
                ...
                >
                    <div
                        ...
                        <button
                            ...
                            disabled={validating || launchingBrowserLogin || importingBrowserLogin}
                        >
                            <X size={18}/>
                        </button>
                    </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="card bg-base-200/40 border border-base-300 mt-4">
                            <div className="card-body py-4 gap-3">
                                <div className="flex items-center justify-between flex-wrap gap-2">
                                    <div>
                                        <div className="font-medium text-sm">{t("accounts.browserLoginTitle")}</div>
                                        <div className="text-xs text-base-content/60 mt-1">{t("accounts.browserLoginDesc")}</div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label className="label cursor-pointer gap-2">
                                            <span className="label-text text-xs">{t("accounts.browserProfileMode")}</span>
                                            <select
                                                className="select select-bordered select-xs"
                                                value={browserProfileMode}
                                                onChange={(e) => setBrowserProfileMode(e.target.value as "main" | "isolated")}
                                                disabled={launchingBrowserLogin || importingBrowserLogin || validating}
                                            >
                                                <option value="main">{t("accounts.browserProfileMain")}</option>
                                                <option value="isolated">{t("accounts.browserProfileIsolated")}</option>
                                            </select>
                                        </label>
                                        <button
                                            className="btn btn-outline btn-sm gap-2"
                                            ...
                                            disabled={launchingBrowserLogin || importingBrowserLogin || validating}
                                        >
                                            <Globe size={14} className={launchingBrowserLogin ? "animate-pulse" : ""}/>
                                            {launchingBrowserLogin ? t("accounts.browserOpening") : t("accounts.browserOpen")}
                                        </button>
                                        <button
                                            className="btn btn-primary btn-sm gap-2"
                                            onClick={handleImportFromBrowserLogin}
                                            disabled={!browserLoginInfo || importingBrowserLogin || launchingBrowserLogin || validating}
                                        >
                                            <LogIn size={14} className={importingBrowserLogin ? "animate-pulse" : ""}/>
                                            {importingBrowserLogin ? t("accounts.browserImporting") : t("accounts.browserImport")}
                                        </button>
                                    </div>
                                </div>
                                ...
                                {browserLoginInfo && (
                                    <div className="text-xs text-base-content/70 space-y-1">
                                        <div>{t("accounts.browserPort")} {browserLoginInfo.debug_port}</div>
                                        <div>{t("accounts.browserProfilePath")} {browserLoginInfo.profile_path}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            ...
                            <button
                                ...
                                onClick={handleSubmitAccountModal}
                                disabled={validating || launchingBrowserLogin || importingBrowserLogin}
                            >
                        ...
                        <div className="flex items-center gap-1">
                            {editingApiKeyId === account.id ? (
                              ...
                              <Copy size={10} className="opacity-40" />
                                    ...
                                    <button
                                        className="btn btn-ghost btn-xs p-0 min-h-0 h-auto"
                                        onClick={() => void openApiKeySwitcher(account)}
                                        title={text("Switch site key", "Switch site key")}
                                        type="button"
                                    >
                                        <ArrowLeftRight size={14} className="opacity-60 hover:opacity-100"/>
                                    </button>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center justify-between border-b border-base-300 px-5 py-4">
                      <div>
                          <h3 className="text-lg font-semibold">
                              {text("Switch Site API Key", "Switch Site API Key")}
                          </h3>
                          <p className="text-sm text-base-content/50 mt-1">
                              {apiKeySwitcherAccount.site_name} · {apiKeySwitcherAccount.site_url}
                          </p>
                      </div>
                      <button className="btn btn-ghost btn-sm" onClick={closeApiKeySwitcher} type="button">
                          <X size={16}/>
                      </button>
                  </div>
                  ...
                  <div className="px-5 py-5 space-y-4">
                      {apiKeySwitcherError && (
                          <div role="alert" className="alert alert-error">
                              <span>{apiKeySwitcherError}</span>
                          </div>
                      )}

                      {apiKeySwitcherLoading ? (
                          <div className="rounded-lg border border-dashed border-base-300 px-4 py-6 text-sm text-base-content/50">
                              {text("Loading site token list...", "Loading site token list...")}
                          </div>
                      ) : apiKeySwitcherTokens.length === 0 ? (
                          <div className="rounded-lg border border-dashed border-base-300 px-4 py-6 text-sm text-base-content/50">
                              {text("No switchable tokens were returned for this site.", "No switchable tokens were returned for this site.")}
                          </div>
                      ) : (
                          <div className="overflow-x-auto">
                              <table className="table table-sm">
                                  ...
                                  <tbody>
                                  {apiKeySwitcherTokens.map((token) => {
                                      const rawKey = tokenKeyValue(token);
                                      const currentKey = apiKeySwitcherAccount.account_info.api_key || "";
                                      const current = rawKey && rawKey === currentKey;
                                      const usable = Boolean(rawKey) && !rawKey.includes("*");
                                      const displayName =
                                          toTokenText(token.name) ||
                                          toTokenText(token.token_name) ||
                                          tokenId(token);
                                      return (
                                          <tr key={tokenId(token)}>
                                              <td className="max-w-[220px] truncate" title={displayName}>
                                                  {displayName}
                                              </td>
                                              <td className="font-mono text-xs">{maskTokenKey(rawKey)}</td>
                                              <td>
                                                  <span className={`badge badge-sm ${current ? "badge-success" : usable ? "badge-ghost" : "badge-warning"}`}>
                                                      {current
                                              ...
                                              <td>
                                                  <button
                                                      ...
                                                      onClick={() => void applySwitchedApiKey(token)}
                                                      disabled={!usable || current || apiKeySwitcherApplying === tokenId(token)}
                                                      type="button"
                                                  >
                                                      {current
                                                          ? text("In use", "In use")
                                                          : apiKeySwitcherApplying === tokenId(token)
                                                              ? text("Switching...", "Switching...")
                                                              : text("Use this key", "Use this key")}
                                                  </button>
                                              </td>
                                          </tr>
                                      );
