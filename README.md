...
APIManager 是一个基于 Tauri 2、React 和 Rust 构建的桌面应用，目标是把分散在多个 AI API 中转站、聚合站、兼容 OpenAI/Anthropic/Gemini 协议的站点资源统一管理起来，并通过本地端口提供一个稳定、可观测、可控的统一代理入口。
...
APIManager 并不是简单把多个 Key 拼在一起，而是把“账号管理”和“代理转发”拆成两层：
...
APIManager 支持为“本地访问 Key”配置权限，而不是把所有上游能力直接暴露给所有客户端。
